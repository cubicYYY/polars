...
fn cast_list_uint8_to_binary<O: Offset>(list: &ListArray<O>) -> PolarsResult<BinaryViewArray> {
    ...
    // Each shared buffer keeps every view's `offset + length` within `MAX_BUF_SIZE`. Rows
    // longer than that get a dedicated buffer at offset 0; rows above `MAX_ROW_LEN` are
    // rejected. Tests shrink the bounds so both paths are exercised without 4GB allocations.
    #[cfg(not(test))]
    ...
    for index in 0..list.len() {
        ...
        polars_ensure!(
            length <= MAX_ROW_LEN,
            InvalidOperation:
            "when casting to BinaryView, list lengths must be <= {MAX_ROW_LEN}"
        );
        ...
        if length > MAX_BUF_SIZE {
            std::hint::cold_path();
            // Slice the active buffer into [pre-row | row | rest], where the middle slice is
            // the dedicated buffer for this oversize row.
            ...
            let (oversize, next) = rest.split_at(length);

            if !current.is_empty() {
                cloned_buffers.push(current);
                buf_index += 1;
            }
            cloned_buffers.push(oversize);
            let oversize_idx = buf_index;
            cloned_buffers.push(next);
            buf_index += 1;
            previous_buf_lengths = end;

            // SAFETY: length > MAX_BUF_SIZE >= View::MAX_INLINE_SIZE so this view is non-inline.
            let view = unsafe {
        ...
        if end - previous_buf_lengths > MAX_BUF_SIZE {
            // Next row's end-offset would overflow `MAX_BUF_SIZE`; rotate to a fresh slice.
            std::hint::cold_path();
            buf_index += 1;
...
mod tests {
    ...
    /// Rows whose length is larger than `MAX_BUF_SIZE` (the i32 offset limit) but still within
    /// `MAX_ROW_LEN` (the u32-1 row limit) are placed in a dedicated buffer with `offset == 0`.
    #[test]
    fn cast_list_uint8_to_binary_oversize_row_gets_dedicated_buffer() {
        // 20 bytes per row: > MAX_BUF_SIZE (15) but <= MAX_ROW_LEN (30) under cfg(test).
        let mut data: Vec<u8> = (0..20).collect();
        data.extend(20..40); // second oversize row, 20 bytes.
        let values = PrimitiveArray::from_slice(data).boxed();
        let dtype =
            ArrowDataType::List(Box::new(Field::new("".into(), ArrowDataType::UInt8, true)));
        let list_u8 = ListArray::try_new(
            dtype,
            unsafe { OffsetsBuffer::new_unchecked(vec![0, 20, 40].into()) },
            values,
            None,
        )
        .unwrap();
        ...
        // Both views must reference offset 0 in their own dedicated oversize buffers.
        for view in binary_array.views().iter() {
            assert_eq!(view.offset, 0);
        }
    }

    /// Rows that exceed `MAX_ROW_LEN` (u32::MAX - 1 in production, 30 under cfg(test)) are
    /// rejected outright.
    #[test]
    fn cast_list_uint8_to_binary_errors_row_above_max_row_len() {
        // 31 bytes is one over MAX_ROW_LEN (30) under cfg(test).
        let values = PrimitiveArray::from_slice(vec![0u8; 31]);
        let dtype =
            ArrowDataType::List(Box::new(Field::new("".into(), ArrowDataType::UInt8, true)));
        let list_u8 = ListArray::new(
            dtype,
            OffsetsBuffer::one_with_length(31),
            values.boxed(),
            None,
        );
        ...
        assert!(matches!(
            ...
            PolarsError::InvalidOperation(msg)
                if msg.as_ref() == "when casting to BinaryView, list lengths must be <= 30"
        ));
    }
