...
impl<T: ViewType + ?Sized> MutableBinaryViewArray<T> {
    ...
    pub fn push_value_into_buffer(&mut self, bytes: &[u8]) -> View {
        ...
        // Oversized rows go in a dedicated buffer at offset 0.
        if bytes.len() > MAX_BUFFER_LEN {
            ...
            self.completed_buffers.push(Buffer::from(bytes.to_vec()));
            // SAFETY: bytes.len() > View::MAX_INLINE_SIZE was established above.
            return unsafe { View::new_noninline_unchecked(bytes, buffer_idx, 0) };
        }
...
mod tests {
    ...
    /// `push_buffer` produces the same control flow as the oversize cold path in
    /// `push_value_into_buffer` (both flush `in_progress` if non-empty, then push the new
    /// buffer to `completed_buffers`). Verify that pushing such a buffer followed by a
    /// regular non-inline value does NOT leave an empty / obsolete buffer in the result.
    #[test]
    fn oversize_buffer_then_small_value_no_obsolete_buffer() {
        ...
        bv.push_value_ignore_validity(b"a long enough string to be non-inline");
        let arr: BinaryViewArrayGeneric<[u8]> = bv.into();
        ...
        assert_eq!(arr.value(0), &oversize[..]);
        assert_eq!(arr.value(1), b"a long enough string to be non-inline");
        assert_eq!(arr.data_buffers().len(), 2);
    }

    /// Round trip "small, oversize, small" and confirm no empty buffers are produced.
    #[test]
    fn small_oversize_small_no_obsolete_buffer() {
        ...
        unsafe { bv.views_mut() }.push(oversize_view);
        bv.total_bytes_len += oversize.len();
        bv.push_value_ignore_validity(b"trailing non-inline string here");
        let arr: BinaryViewArrayGeneric<[u8]> = bv.into();
        for (i, buf) in arr.data_buffers().iter().enumerate() {
            assert!(
                !buf.is_empty(),
        ...
        assert_eq!(arr.value(1), &oversize[..]);
